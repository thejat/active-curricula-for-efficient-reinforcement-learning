...
def plot_policy(Q, num):
	...
	# plt.imshow(plot, interpolation='none', cmap='Blues')\
	plt.clf()
	fig = plt.imshow(plot, interpolation='nearest',cmap='Blues')
	# plt.axis('off')
	fig.axes.get_xaxis().set_visible(False)
	fig.axes.get_yaxis().set_visible(False)
	if num == 0:
		plt.savefig("policies/final_policy.png", bbox_inches='tight', pad_inches = 0)
	else:
		plt.savefig("policies/policy_%d.png" % (num), bbox_inches='tight', pad_inches = 0)
...
if __name__ == "__main__":
	...
	env = Grid(grid_size, block_states, goal, start)
