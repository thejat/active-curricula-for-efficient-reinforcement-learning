...
class Maze(object):
	...
	def draw(self, comb, perm, task):
		...
		plt.savefig("comb/tasks/comb_%d_%d_%d.png" % (comb, perm, task))
...
if __name__ == "__main__":
	...
	grid_size = 7
	print ('total no. of tasks: ',len(subtasks)+1)
	# perm_episodes = []
	tot_steps = 2500000

	for number_of_tasks in range(1, len(subtasks)+1):
		# if(number_of_tasks != 3):
		# 	continue
		perm = 0              # to track no. of permutations
		print ('no. of tasks in combination: ', number_of_tasks)
		for c in itertools.combinations(subtasks, number_of_tasks):
			C = list(c)
			for T in itertools.permutations(C):
				...
				while(task < no_tasks):
					...
					env.draw(number_of_tasks, perm, task)
				...
				plot =  [[max(Q[i][j]) for i in range(grid_size)] for j in range(grid_size)]
				plt.imshow(plot, interpolation='none', cmap='gray')
				plt.savefig("comb/policies/%d_%d_policy.png" % (number_of_tasks, perm))
				...
				plt.savefig('comb/resutl%d_%d_1.png' % (number_of_tasks, perm))
				print ('done')
				print('plotting')
				...
				# plt.yscale('log')
				# plt.gca().invert_yaxis()
				plt.savefig('comb/resutl%d_%d_2.png' % (number_of_tasks, perm))
				print ('done')
	...
	plt.figure(1)
	plt.savefig('comb/result_1.png')
	plt.figure(2)
	plt.savefig('comb/result_2.png')
